...
namespace Hash
{
	...
	template<typename T>
	inline uint32_t Crc32(const uint32_t crc32, const T& data)
	{
		return Crc32(crc32, reinterpret_cast<const uint8_t*>(&data), sizeof(T));
	}

	template<typename T>
	inline uint32_t Crc32(const T& data)
	{
		return Crc32(reinterpret_cast<const uint8_t*>(&data), sizeof(T));
	}

	template<>
	inline uint32_t Crc32<std::string>(const std::string& data)
	{
	...
	template<>
	inline uint32_t Crc32<std::string>(uint32_t crc32, const std::string& data)
	{
		static_assert(sizeof(uint8_t) == sizeof(char));
